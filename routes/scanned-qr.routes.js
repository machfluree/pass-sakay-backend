      ...
      const promise = new Promise((resolve, reject) => {
        ...
        bodyParams.forEach(async (param, index, arr) => {
          ...
          if (param.passengerAccount) {
            query.passengerAccount = {
              $ne: mongoose.Types.ObjectId(param.passengerAccount),
            };
          }
          if (param.tripSched) {
          ...
          const tripHistory = await ScannedQr.find(query).populate(
            "passengerAccount",
          );
          if (tripHistory) {
...
route.post(
  "/trip/boarding/time-in",
  [getTimeInMiddleware],
  async (req, res) => {
    try {
      if (res.scannedQR) {
        res.status(400).json({ message: "Passenger already boarded." });
      } else {
        const {
          passengerAccount,
          busAccount,
        ...
        if (newScannedQR) {
          res.status(201).json({
            message: "Scanned QR successfully saved.",
            newRecord: newScannedQR,
          });
        } else {
          res.status(400).json({ message: "Scanned QR failed to save." });
        }
...
route.put(
  "/trip/boarding/time-out",
  [getTimeInMiddleware],
  async (req, res) => {
    try {
      // const { timeOut } = req.body;
      // const scannedQR = await ScannedQr.findById(req.params.id);
      if (res.scannedQR) {
        res.scannedQR.timeOut = Date.now();
        res.scannedQR.timeOutStatus = true;
        res.scannedQR.landmarkOut = req.body.landmarkOut;
        res.scannedQR.tripPlaceOfScanOut = req.body.tripPlaceOfScanOut;
        const updateForTimeOut = await res.scannedQR.save();
        res.status(200).json(updateForTimeOut);
      } else {
        res.status(400).json({ message: "Passenger hasn't boarded yet." });
      }
    } catch (error) {
      console.error(error);
...
// update a user
route.put("/scanned-qr/:id", [getScannedQRMiddleware], async (req, res) => {
...
async function getTimeInMiddleware(req, res, next) {
  let scannedQR;
  try {
    let query = {
      leave: { $exists: false },
    ...
    if (req.body.date) {
      query.date = {
        $gte: moment(req.body.date).hours(0).minutes(0).milliseconds(0),
        $lte: moment(req.body.date).hours(59).minutes(59).milliseconds(59),
      };
    }
    if (req.body.timeOut) {
      query.timeIn = { $lt: moment(req.body.timeOut) };
    } else {
      query.timeIn = { $lt: moment(Date.now()) };
    }
    query.timeInStatus = { $eq: true };

    scannedQR = await ScannedQr.findOne(query);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error_message: error.message });
  }
